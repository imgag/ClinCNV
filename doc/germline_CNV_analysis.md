...
2. Specifying output folder:
`R clinCNV.R --normal normal.cov --bed bedFile.bed --out /your/folder`

3. Addinog offtarget coverages (does not increase _Sensitivity_ a lot since germline CNVs are rarely as long as off-target window, but improves _Specificity_ efficiently removing CNVs formed by probes standing far away from each other):
