      ...
      while (counter <= nrow(regionsToLookForMCNVs)) {
        if (regionsToLookForMCNVs[counter,3] - regionsToLookForMCNVs[counter,2] < 2) {
          ...
          for (j in (regionsToLookForMCNVs[counter,2] + 1):(regionsToLookForMCNVs[counter,3] - 2)) {
            print(j)
            if (!checkConnectivityMed(coverageToWorkWith[j,], coverageToWorkWith[j + 1,], multipliersSamplesForAnalysisDiscovery, localSdsOfProbes[j], localSdsOfProbes[j + 1], medianCorrelation, percentageToBePolymorphism)) {
              if (j + 2 <= nrow(coverageToWorkWith)) {
                if (checkConnectivityMed(coverageToWorkWith[j,], coverageToWorkWith[j + 2,], multipliersSamplesForAnalysisDiscovery, localSdsOfProbes[j], localSdsOfProbes[j + 1], medianCorrelation, percentageToBePolymorphism)) {
                  next
                }
              }
