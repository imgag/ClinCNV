...
option_list = list(
  make_option(c("-i", "--input"), type="character", default=NULL, 
              help="folder with .cov files that have to be merged", metavar="character"),
...
if (is.null(opt$input) | is.null(opt$out) | is.null(opt$numOfColumn)){
  ...
  stop("All arguments has to be specified. Quit.", call.=FALSE)
}
...
merge_covs_from_folder <- function(folder, outputFileName, columnWhereCoveragesStart) {
  ...
  mergeFilesIntoOne <- function(indices) {
    if (length(indices) > 1) {
      ...
      #print(head(matrixOfMergedCoverages))
    } else {
      print("WTF")
    }
