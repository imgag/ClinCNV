...
merge_covs_from_folder <- function(folder, outputFileName, columnWhereCoveragesStart) {
  ...
  mergeFilesIntoOne <- function(indices) {
    if (length(indices) > 1) {
      ...
      print(filenames[indices[1]])
      matrixOfMergedCoverages = (fread(filenames[indices[1]], header=T))
      print(nrow(matrixOfMergedCoverages))
      colnames(matrixOfMergedCoverages)[1] = "X.chr"
    } else {
      print("WTF")
    }
  ...
  coverages <- data.frame(mergeFilesIntoOne(1:length(filenames)))
